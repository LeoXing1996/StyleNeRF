...
def copy_all_python_files(source,
                          snapshot_main_dir,
                          code_snapshot_hash,
                          recurse_dirs="fairseq"):
    """
    ...
    assert not os.path.exists(
        destination), "Code snapshot: {0} alredy exists".format(
            code_snapshot_hash)
    ...
    def all_pys(recurse_dirs):
        ...
        for d in recurse_dirs.split(","):
            yield from iglob(os.path.join(source, d, "**/*.py"),
                             recursive=True)
            yield from iglob(os.path.join(source, d, "**/*.so"),
                             recursive=True)
            yield from iglob(os.path.join(source, d, "**/*.yaml"),
                             recursive=True)
    ...
    for filepath in all_pys(recurse_dirs):
        ...
        shutil.copy2(os.path.join(source, filepath),
...
def launch_cluster(slurm_args, model_args):
    ...
    if slurm_args.get('workplace') is not None:
        ...
        train_stderr = os.path.join(slurm_args['workplace'],
                                    'train.%A.stderr.%j')
    else:
    ...
    train_cmd = [
        'python',
        os.path.join(destination, 'run_train.py'),
    ]
    ...
    sbatch_cmd = [
        'sbatch', '--job-name', jobname, '--partition',
        slurm_args.get('partition', 'learnfair'), '--gres',
        'gpu:volta:{}'.format(gpus), '--nodes',
        str(nodes), '--ntasks-per-node', '1', '--cpus-per-task', '20',
        '--output', train_log, '--error', train_stderr, '--open-mode',
        'append', '--signal', 'B:USR1@180', '--time',
        slurm_args.get('time', '4320'), '--mem',
        slurm_args.get('mem', '500gb'), '--exclusive', '--exclude',
        'learnfair5035,learnfair5289,learnfair5088,learnfair5028,learnfair5032,learnfair5033,learnfair5056,learnfair5098,learnfair5122,learnfair5124,learnfair5156,learnfair5036,learnfair5258,learnfair5205,learnfair5201,learnfair5240,learnfair5087,learnfair5119,learnfair5246,learnfair7474,learnfair7585,learnfair5150,learnfair5166,learnfair5215,learnfair5142,learnfair5070,learnfair5236,learnfair7523'
    ]
