...
def get_random_pose(range_u,
                    ...
                    gaussian=False,
                    angular=False):
    loc, (u, v) = sample_on_sphere(range_u,
                                   range_v,
                                   size=(batch_size),
                                   gaussian=gaussian,
                                   angular=angular)
    radius = range_radius[0] + torch.rand(batch_size) * (range_radius[1] -
                                                         range_radius[0])
...
def get_camera_pose(range_u,
    ...
    loc, _ = sample_on_sphere((u, u), (v, v),
                              size=(batch_size),
                              angular=angular)
...
def get_camera_pose_v2(range_u,
                       range_v,
                       range_r,
                       mode,
                       invert=False,
...
def sample_on_sphere(range_u=(0, 1),
                     range_v=(0, 1),
                     size=(1, ),
                     to_pytorch=True,
                     gaussian=False,
...
def look_at(eye,
    ...
    if not isinstance(eye, torch.Tensor):
        ...
        #   --> but this is wrong, so sad, up should be [0, 1, 0]
        ...
        r_mat = np.concatenate((x_axis.reshape(
            -1, 3, 1), y_axis.reshape(-1, 3, 1), z_axis.reshape(-1, 3, 1)),
                               axis=2)
        if to_pytorch:
        ...
        at, up = torch.from_numpy(at).float().to(
            eye.device), torch.from_numpy(up).float().to(eye.device)
        z_axis = normalize(eye - at[None, :])
        x_axis = normalize(
            torch.cross(up[None, :].expand_as(z_axis), z_axis, dim=-1))
...
def get_corner_rays(corner_pixels, camera_matrices, res):
    ...
    corner_rays = corner_rays.reshape(batch_size, res + 1, res + 1,
                                      6).permute(0, 3, 1, 2)
    corner_rays = torch.cat([
        corner_rays[..., :-1, :-1], corner_rays[..., 1:, :-1],
        corner_rays[..., 1:, 1:], corner_rays[..., :-1, 1:]
    ], 1)
    return corner_rays
...
def arange_pixels(resolution=(128, 128),
    ...
    pixel_scaled = torch.stack([x, y], -1).permute(1, 0,
                                                   2).reshape(1, -1, 2).repeat(
                                                       batch_size, 1, 1)
...
def transform_to_camera_space(p_world,
                              world_mat,
                              camera_mat=None,
                              scale_mat=None):
...
def image_points_to_camera(image_points,
                           camera_mat,
                           invert=False,
                           negative_depth=True,
                           use_absolute_depth=True):
...
def camera_points_to_image(camera_points,
                           camera_mat,
                           invert=False,
                           negative_depth=True,
                           use_absolute_depth=True):
...
def angular_interpolation(res, camera_mat):
    batch_size = camera_mat.shape[0]
    device = camera_mat.device
    input_rays = image_points_to_camera(
        arange_pixels((res, res), batch_size, invert_y_axis=True).to(device),
        camera_mat)
    output_rays = image_points_to_camera(
        arange_pixels((res * 2, res * 2), batch_size,
...
def camera_9d_to_16d(d9):
    ...
    RT = torch.eye(4).to(device=d9.device,
                         dtype=d9.dtype).reshape(1, 4,
                                                 4).repeat(d6.size(0), 1, 1)
    RT[:, :3, :3] = rotation
    RT[:, :3, -1] = translation
...
def depth2pts_outside(ray_o, ray_d, depth):
    ...
    # now rotate p_sphere
    # Rodrigues formula: https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula  # noqa
...
def intersect_sphere(ray_o, ray_d, radius=1):
    '''
    ray_o, ray_d: [..., 3]
    compute the depth of the intersection point between this ray
    and unit sphere
    '''
    # note: d1 becomes negative if this mid point is behind camera
    d1 = -torch.sum(ray_d * ray_o, dim=-1) / torch.sum(ray_d * ray_d, dim=-1)
...
def sample_pdf(bins, weights, N_importance, det=False, eps=1e-5):
    ...
    # denom equals 0 means a bin has weight 0, in which case it
    # will not be sampled
    denom[denom < eps] = 1
...
def normalization_inverse_sqrt_dist_centered(x_in_world, view_cell_center,
                                             max_depth):
