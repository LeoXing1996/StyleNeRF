...
def local_ensemble(pi, po, resolution):
    ...
    ib = torch.tensor([min((i + 1) // 2, resolution // 2 - 1)
                       for i in ii]).long()
...
def get_initial_rays_trig(num_steps,
                          fov,
                          resolution,
                          ray_start,
                          ray_end,
    ...
    x = x.T.flatten()
    y = y.T.flatten()
    z = -torch.ones_like(x, device=device) / math.tan(
        (2 * math.pi * fov / 360) / 2)

    rays_d_cam = normalize_vecs(torch.stack([x, y, z], -1))

    z_vals = torch.linspace(ray_start, ray_end, num_steps,
                            device=device).reshape(1, num_steps,
                                                   1).repeat(W * H, 1, 1)
...
def sample_camera_positions(device,
    ...
    """
    if mode == 'uniform':
        theta = (torch.rand((n, 1), device=device) -
                 0.5) * 2 * horizontal_stddev + horizontal_mean
        phi = (torch.rand(
            (n, 1), device=device) - 0.5) * 2 * vertical_stddev + vertical_mean

    elif mode == 'normal' or mode == 'gaussian':
        theta = torch.randn(
            (n, 1), device=device) * horizontal_stddev + horizontal_mean
        phi = torch.randn(
            (n, 1), device=device) * vertical_stddev + vertical_mean

    elif mode == 'hybrid':
        if random.random() < 0.5:
            theta = (torch.rand((n, 1), device=device) -
                     0.5) * 2 * horizontal_stddev * 2 + horizontal_mean
            phi = (torch.rand((n, 1), device=device) -
                   0.5) * 2 * vertical_stddev * 2 + vertical_mean
        else:
            theta = torch.randn(
                (n, 1), device=device) * horizontal_stddev + horizontal_mean
            phi = torch.randn(
        ...
        phi = torch.ones(
            (n, 1), device=device, dtype=torch.float) * vertical_mean
        theta = torch.ones(
            (n, 1), device=device, dtype=torch.float) * horizontal_mean

    phi = torch.clamp(phi, 1e-5, math.pi - 1e-5)

    output_points = torch.zeros(
        (n, 3), device=device
    )  # torch.cuda.FloatTensor(n, 3).fill_(0)#torch.zeros((n, 3))
...
def perturb_points(points, z_vals, ray_directions, device):
    ...
    offset = (torch.rand(z_vals.shape, device=device) -
...
def transform_sampled_points(points,
    ...
    transformed_points = torch.bmm(
        cam2world_matrix,
        points_homogeneous.reshape(n, -1, 4).permute(0, 2, 1)).permute(
            0, 2, 1).reshape(n, num_rays, num_steps, 4)
    transformed_ray_directions = torch.bmm(
        cam2world_matrix[..., :3, :3],
        ray_directions.reshape(n, -1, 3).permute(0, 2,
    ...
    transformed_ray_origins = torch.bmm(
        cam2world_matrix,
        homogeneous_origins).permute(0, 2, 1).reshape(n, num_rays, 4)[..., :3]
    return transformed_points[..., : 3], z_vals, transformed_ray_directions, \
        transformed_ray_origins, pitch, yaw


def integration(rgb_sigma,
                z_vals,
                device,
                noise_std=0.5,
                last_back=False,
    ...
    for xi, xs in enumerate(X):
        for yi, ys in enumerate(X):
            for zi, zs in enumerate(X):
                ...
                pts = torch.stack([xx, yy, zz], dim=-1).unsqueeze(0).to(
                    styles.device)  # B, H, H, H, C
                block_shape = [1, len(xs), len(ys), len(zs)]
                feat_out, sigma_out = nerf.fg_nerf.forward_style2(pts,
                                                                  None,
                                                                  block_shape,
                                                                  ws=styles)
                sigma_np[
                    xi * block_resolution: xi * block_resolution + len(xs),
                    yi * block_resolution: yi * block_resolution + len(ys),
                    zi * block_resolution: zi * block_resolution + len(zs)] = \
                    sigma_out.reshape(block_shape[1:]).detach().cpu().numpy()
...
def render_mesh(meshes, camera_matrices, render_noise=True):
    ...
    c2w = torch.matmul(
        ...
        torch.diag(torch.tensor(
            [-1.0, 1.0, -1.0, 1.0],
            device=device))[None, :, :])  # Different camera model...
    ...
    for res, (mesh, face_vert_noise) in meshes.items():
        ...
        for i in range(len(poses)):
            ...
            color = interpolate_face_attributes(pix_to_face, bary_coord,
                                                face_vert_noise).squeeze()
...
def generate_option_outputs(render_option):
    ...
    if ('depth' in render_option.split(',')):
        ...
        if 'gradient' in render_option.split(','):
            points = (camera_world[:, :, None] +
                      di[:, :, :, None] * ray_vector[:, :, None]).reshape(
                          batch_size, tgt_res, tgt_res, di.size(-1), 3)
            with torch.enable_grad():
                gradients = self.fg_nerf.forward_style2(
                    points,
                    None, [batch_size, tgt_res,
                           di.size(-1), tgt_res],
                    get_normal=True,
                    ws=styles,
                    z_shape=z_shape_obj,
                    z_app=z_app_obj).reshape(batch_size, di.size(-1), 3,
                                             tgt_res * tgt_res).permute(
                                                 0, 3, 1, 2)
        ...
        if 'value' in render_option.split(','):
            ...
            img = reformat(fg_feat.repeat(1, 1, 3),
                           tgt_res) / fg_feat.max() * 2 - 1
