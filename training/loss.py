    ...
    def set_alpha(self, steps):
        ...
        if self.curriculum is not None and self.curriculum != 'None':
            if self.curriculum == 'upsample':
                ...
                assert len(self.curriculum
                           ) == 2, "currently support one stage for now"
        ...
        if (generator_mode == 'image_z_random_c') or (generator_mode
            ...
            if generator_mode == 'image_z_image_c':
                with misc.ddp_sync(self.D, False):
                    synthesis_kwargs['camera_RT'] = misc.get_func(
                        self.D, 'get_estimated_camera')[0](img)
            with misc.ddp_sync(self.G_synthesis, sync):
                out = self.G_synthesis(ws, **synthesis_kwargs)
            if get_loss:
                # consistency loss given the image predicted camera
                # (train the image encoder jointly)
                out['consist_l1_loss'] = F.smooth_l1_loss(
                    out['img'], img['img']) * 2.0  # TODO: DEBUG
            ...
            with misc.ddp_sync(self.G_mapping, sync):
                ...
                if self.style_mixing_prob > 0:
                    with torch.autograd.profiler.record_function(
                        ...
                        cutoff = torch.empty([],
                                             dtype=torch.int64,
                                             device=ws.device).random_(
                                                 1, ws.shape[1])
                        cutoff = torch.where(
                            torch.rand([], device=ws.device) <
                            self.style_mixing_prob, cutoff,
                            torch.full_like(cutoff, ws.shape[1]))
                        ws[:, cutoff:] = self.G_mapping(
                            torch.randn_like(z), c,
            ...
            if generator_mode == 'random_z_image_c':
                ...
                with torch.no_grad():
                    ...
                    with misc.ddp_sync(D, sync):
                        estimated_c = misc.get_func(
                            D, 'get_estimated_camera')(img)[0].detach()
    ...
    def accumulate_gradients(self,
        ...
        if do_Gmain:
            ...
            with torch.autograd.profiler.record_function('Gmain_backward'):
                ...
                loss = scaler.scale(
                    losses['Gmain']) if scaler is not None else losses['Gmain']
                loss.backward()
        ...
        if do_Gpl and (self.pl_weight != 0):
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                gen_img, gen_ws = self.run_G(gen_z[:batch_size],
                                             gen_c[:batch_size],
                                             sync=sync,
                                             img=fake_img[:batch_size]
                                             if fake_img is not None else None)
        ...
        if do_Dmain:
            ...
            with torch.autograd.profiler.record_function('Dgen_backward'):
                ...
                loss = scaler.scale(
                    losses['Dgen']) if scaler is not None else losses['Dgen']
                loss.backward()
        ...
        if do_Dmain or (do_Dr1 and (self.r1_gamma != 0)):
            name = 'Dreal_Dr1' if do_Dmain and do_Dr1 \
                else 'Dreal' if do_Dmain else 'Dr1'
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if do_Dmain:
                    ...
                    if self.label_smooth > 0:
                        loss_Dreal = loss_Dreal * (
                            1 -
                            self.label_smooth) + torch.nn.functional.softplus(
                                real_logits) * self.label_smooth

                    training_stats.report('Loss/D/loss',
                                          loss_Dgen.mean() + loss_Dreal.mean())

                loss_Dr1 = 0
                if do_Dr1:
                    with torch.autograd.profiler.record_function(
                            'r1_grads'), conv2d_gradfix.no_weight_gradients():
                        real_img_tmp = real_img['img'] if isinstance(
                            real_img, dict) else real_img
            ...
            with torch.autograd.profiler.record_function(name + '_backward'):
                losses['Dr1'] = (real_logits * 0 + loss_Dreal +
                                 loss_Dr1).mean().mul(gain)
                loss = scaler.scale(
