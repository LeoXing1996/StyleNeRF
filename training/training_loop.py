...
def training_loop(
    ...
    data_loader_kwargs={},  # Options for torch.utils.data.DataLoader.  # noqa
    ...
    world_size=1,  # Number of GPUs participating in the training.  # noqa
    rank=0,  # Rank of the current process.
    gpu=0,  # Index of GPU used in training
    batch_gpu=4,  # Batch size for once GPU
    batch_size=4,  # Total batch size for one training iteration. Can be larger than batch_gpu * world_size.  # noqa
    ema_kimg=10,  # Half-life of the exponential moving average (EMA) of generator weights.  # noqa
    ema_rampup=None,  # EMA ramp-up coefficient.
    G_reg_interval=4,  # How often to perform regularization for G? None = disable lazy regularization.  # noqa
    D_reg_interval=16,  # How often to perform regularization for D? None = disable lazy regularization.  # noqa
    augment_p=0,  # Initial value of augmentation probability.  # noqa
    ...
    resume_pkl=None,  # Network pickle to resume training from.  # noqa
    resume_start=0,  # Resume from steps
    cudnn_benchmark=True,  # Enable torch.backends.cudnn.benchmark?  # noqa
    allow_tf32=False,  # Enable torch.backends.cuda.matmul.allow_tf32 and torch.backends.cudnn.allow_tf32?  # noqa
    ...
    update_cam_prior_ticks=None,  # (optional) Non-parameteric updating camera poses of the dataset  # noqa
    ...
    **unused,
    ...
    torch.backends.cuda.matmul.allow_tf32 = allow_tf32  # Allow PyTorch to internally use tf32 for matmul  # noqa
    torch.backends.cudnn.allow_tf32 = allow_tf32  # Allow PyTorch to internally use tf32 for convolutions  # noqa
    ...
    if generation_with_image:
        backup_data_iterator = iter(
            torch.utils.data.DataLoader(dataset=copy.deepcopy(training_set),
                                        sampler=training_set_sampler,
                                        batch_size=batch_size // world_size,
                                        **data_loader_kwargs))
    ...
    ddp_modules = dict()
    module_list = [('G_mapping', G.mapping), ('G_synthesis', G.synthesis),
    ...
    for name, module in module_list:
        if (world_size > 1) and (module is not None) and len(
            ...
            module = torch.nn.parallel.DistributedDataParallel(
                ...
                find_unused_parameters=True)  # allows progressive
    ...
    if rank == 0:
        ...
        try:
            if slurm:
                ...
                client = FileClient('petrel',
                                    path_mapping=dict(petrel_mapping))
            else:
    ...
    if rank == 0:
        ...
        grid_i = (torch.from_numpy(images).float() / 127.5 -
                  1).to(device).split(batch_gpu)

        if not os.path.exists(os.path.join(img_dir, 'reals.png')):
            save_image_grid(images,
                            os.path.join(img_dir, 'reals.png'),
                            drange=[0, 255],
                            grid_size=grid_size,
                            client=client)
        ...
        if not os.path.exists(os.path.join(img_dir, 'fakes_init.png')):
            ...
            save_image_grid(images,
                            os.path.join(img_dir, 'fakes_init.png'),
                            drange=[-1, 1],
                            grid_size=grid_size,
                            client=client)
    ...
    while True:
        ...
        if hasattr(training_set_sampler, 'update_dataset_cameras') and \
            (cur_nimg == resume_start and resume_start > 0) and \
                (cur_tick > update_cam_prior_ticks):
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            def load_data(iterator):
                ...
                img = [{
                    'img': img
                } for img in (img.to(device).to(torch.float32) / 127.5 -
                              1).split(batch_gpu)]
            ...
            all_gen_img = [[None for _ in range(len(phase_real_img))]
        ...
        for phase, phase_gen_z, phase_gen_c, phase_gen_img in zip(
            ...
            for round_idx, (real_img, real_c, gen_z, gen_c,
                            fake_img) in enumerate(
                                zip(phase_real_img, phase_real_c, phase_gen_z,
                                    phase_gen_c, phase_gen_img)):
                sync = (round_idx == batch_size // (batch_gpu * world_size) -
                        1)
                gain = phase.interval

                losses = loss.accumulate_gradients(phase=phase.name,
                                                   real_img=real_img,
                                                   real_c=real_c,
                                                   gen_z=gen_z,
                                                   gen_c=gen_c,
            ...
            with torch.autograd.profiler.record_function(phase.name + '_opt'):
                if len(losses) > 0:
                    ...
                    if phase.scaler is not None:
                        ...
                        phase.scaler.update()
                        training_stats.report(f'Scaler/{phase.name}',
                                              phase.scaler.get_scale())
                    else:
                        phase.opt.step()
        ...
        if loss.alpha is not None:
            fields += [
                f"alpha {training_stats.report0('Progress/alpha', loss.alpha):<8.5f}"  # noqa
            ]
            fields += [
        ...
        fields += [
            f"maintenance {training_stats.report0('Timing/maintain_sec', maintenance_time):<5.1f}"  # noqa
        ]
        fields += [
            ...
            with torch.no_grad():
                ...
                images = torch.cat([
                    G_ema.get_final_output(z=z,
                                           ...
                                           noise_mode='const',
                                           img=img,
                                           camera_mode=[0.5, 0.5, 0.5]).cpu()
                    for z, c, img in zip(grid_z, grid_c, grid_i)
                ]).numpy()
                save_image_grid(images,
                                os.path.join(
                                    img_dir,
                                    f'fakes{cur_nimg//1000:06d}_000.png'),
                                drange=[-1, 1],
                                grid_size=grid_size,
                                client=client)
        ...
        if (network_snapshot_ticks
            ...
            if rank == 0:
                if client is not None:
                    ...
                    shutil.copy(
                        snapshot_pkl,
                        os.path.join(run_dir, 'latest-network-snapshot.pkl'))

        # Evaluate metrics.
        if (snapshot_data
                is not None) and (len(metrics) > 0) and (cur_tick > 1):
            if rank == 0:
        ...
        if stats_jsonl is not None:
            ...
            stats_jsonl.flush()
            if rank == 0:
                losses = [(key, fields[key]) for key in fields
                          if 'Loss/' in key]
                losses = [
                    "{}: {:.4f}".format(key[5:], loss['mean'])
